...
#Importing data
data_file_path <- "C:\\Users\\SL3234\\OneDrive - Corporate Network\\Desktop\\Data-20210528"
...
for(field in num_fields[-c(5)]){
  hist(analyse_df[,field], main = paste(field))
}
par(mfrow = c(1,1))

plot_list <- list()
for (i in 1:length(num_fields)){
  plot_list[[i]] <- ggplot(data = train_data, aes(x=num_fields[i])) + geom_histogram(stat="count")
}

grid.arrange(grobs = plot_list, ncol=2)

ggplot(data = train_data, aes(x=Elev)) + geom_histogram(bins = 10)


#Box-Plot of numerical against Stream.Water.Level


#Histogram of numerical variables with colour of Stream.Water.Level

#Frequency table of cat variables with Stream.Water.Level

#Scatter Plot of Stream.Discharge and numerical variables

#Boxplot of Stream.Discharge and catgeorical variables



#Visualising facet relationships
p1 <- ggplot(data = tolerance, aes(x = time, y = tolerance, group = id))
